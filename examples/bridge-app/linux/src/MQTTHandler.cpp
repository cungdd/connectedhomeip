...
MQTTHandler::MQTTHandler(IoTDeviceManager & manager, const std::string & configFile) : mosquittopp(), deviceManager(manager)
{
...
void MQTTHandler::handleAddDevice(const Json::Value & root)
{
    ...
    if (deviceManager.getDevices().isMember(newDevice.id))
        return;
    // IoTDevice newDevice = IoTDevice::fromJson(root["data"]);
...
void MQTTHandler::handleTouchSwitchControl(const Json::Value & root)
{
    ...
    if (state["onoff"] == 1)
    {
        LightSwitchMgr::GetInstance().TriggerLightSwitchAction(LightSwitchMgr::LightSwitchAction::On, false,
                                                               (chip::EndpointId) device.endpoint, 6);
    }
    else
    {
        LightSwitchMgr::GetInstance().TriggerLightSwitchAction(LightSwitchMgr::LightSwitchAction::Off, false,
                                                               (chip::EndpointId) device.endpoint, 6);
    }
}
...
void MQTTHandler::sendControl(const std::string & deviceId, uint8_t * onoff, uint8_t * dim, uint8_t * ctt, uint8_t * h, uint8_t * l,
                              uint8_t * s)
{
    Json::Value root;
    root["cmd"] = "controlDev";
    ...
    if (onoff != nullptr)
        deviceData["onoff"] = Json::Value(*onoff);
    if (dim != nullptr)
        deviceData["dim"] = *dim;
    if (ctt != nullptr)
        deviceData["ctt"] = *ctt;
...
void MQTTHandler::on_message(const struct mosquitto_message * message)
{
    ...
    if (Json::parseFromStream(reader, s, &root, &errs))
    {
        ...
        if (cmd == "add_device")
            handleAddDevice(root);
        else if (cmd == "del_device")
            handleDeleteDevice(root);
        else if (cmd == "controlDev")
            handleLightControl(root);
        else if (cmd == "touch_switch_ctrl")
            handleTouchSwitchControl(root);
        else if (cmd == "sensor_value")
            handleSensorValue(root);
        else if (cmd == "reset")
